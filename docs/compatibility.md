...
- interactive 当前轮渲染：assistant/tool/notice 仅在本轮 `Live` 区中增量显示；本轮结束后统一刷入 scrollback，不再保留固定 transcript 面板。
...
- interactive 多轮对话采用 scrollback 历史 + 当前轮 transient live；用户消息只保留 prompt 提交后的单一来源显示
