...
class AgentSession:
    ...
    def _resolve_model_pattern(self, pattern: str, all_models: list[tuple[str, str]]) -> list[tuple[str, str]]:
        ...
        # Glob patterns can match many entries.
        ...
        # Fuzzy model lookup (prefer alias over dated versions)
        partial = [
            (provider, model)
            for provider, model, token in model_tokens
    ...
    def _emit(self, event: AgentSessionEvent | AgentEvent) -> None:
        event_type = getattr(event, "type", "")
        if event_type:
            try:
                payload = event.model_dump(by_alias=True) if hasattr(event, "model_dump") else {"type": event_type}
                self.extension_runner.emit(event_type, payload, self)
            except Exception:
                pass
    ...
    async def _handle_slash_command(self, text: str) -> tuple[bool, str | None]:
        ...
        if command == "reload":
            diagnostics = self.reload_resources()
        ...
        if command == "model":
            if not rest:
                return True, f"Current model: {self.provider_name}/{self.model_id}"
            model_value, thinking = self._split_model_thinking(rest)
            self.provider_name, self.model_id = self._resolve_provider_and_model(None, model_value)
            self.session_manager.append_model_change(self.provider_name, self.model_id)
            if thinking:
                self.set_thinking_level(thinking)
            return True, f"Model set to {self.provider_name}/{self.model_id}"
        if command == "name":
            if rest:
                self.session_manager.set_session_name(rest)
        ...
        if command == "settings":
            return True, self._handle_settings_command(rest)
        if command == "compact":
            return True, self._handle_manual_compact()
        if command == "tree":
            return True, self._handle_tree_command(rest)
        if command == "fork":
            return True, self._handle_fork_command(rest)
        if command == "resume":
            return True, self._handle_resume_command(rest)
        if command == "scoped-models":
            return True, self._handle_scoped_models_command(rest)

        ext_cmds = self.extension_runner.get_commands()
        if command in ext_cmds:
            result = ext_cmds[command].handler(rest, self)
    ...
    def list_sessions(
        ...
        # Fetch enough rows so client-side filtering/offset stays deterministic.
    ...
    def set_steering_mode(self, mode: str) -> None:
        ...
        self.steering_mode = mode  # runtime override

    def set_follow_up_mode(self, mode: str) -> None:
        if mode not in {"all", "one-at-a-time"}:
            raise ValueError("follow-up mode must be all or one-at-a-time")
        self.follow_up_mode = mode  # runtime override
    ...
    async def _run_with_retry(self, prompts: list[AgentMessage], context: list[AgentMessage]) -> list[AgentMessage]:
    ...
    async def prompt(
        ...
        expanded = self._expand_user_prompt(message)
        handled, response = await self._handle_slash_command(expanded)
        if handled:
        ...
        new_messages = await self._run_with_retry(prompts=prompts, context=context)
        for msg in new_messages:
            self.session_manager.append_message(msg)

        self.extension_runner.emit("agent_end", {"messages": [m.model_dump(by_alias=True) for m in new_messages]}, self)
        return new_messages

    async def continue_run(self) -> list[AgentMessage]:
        context = self.session_manager.build_context().messages
        new_messages = await self._run_with_retry([], context)
        for msg in new_messages:
            self.session_manager.append_message(msg)
        self.extension_runner.emit("agent_end", {"messages": [m.model_dump(by_alias=True) for m in new_messages]}, self)
        return new_messages
